...
}); //Estrutura que lê do console para pode capturar informações 
    ...
    const validarOperador = (operador) => {
        switch (operador) {
            ...
            default:
                ...
                return null // senão for nenhum desses vai dar erro
        }
    }
...
readLine.question('favor informar um número:', numero1 => {
    const numeroValidado1 = validarNumeroInformado(numero1); // primeira leitura do primeiro número

    if (numeroValidado1) {
        readLine.question('Favor informar outro numero:', (numero2) => {
            const numeroValidado2 = validarNumeroInformado(numero2);//captura dos 2 número validos

            if (numeroValidado2) {
                readLine.question('Favor informar o operador: ', (operador) => {
                    const operadorValidado = validarOperador(operador); //captura do operador
                    ...
                    if (operadorValidado) {
                        switch (operadorValidado) {
                            ...
                            case '*': console.log(`Operador selecionado modulo resultado ${numeroValidado1} % ${numeroValidado2} = ${numeroValidado1} % ${numeroValidado2}`);
                                break;
                            default: break;
                        } // se o operadorfor valido executa a conta
                    }
                });
            }
        });
